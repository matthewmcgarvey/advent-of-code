...
function part2(ids) {
  ...
  let desiredLength = ids[0].length - 1;
  return diffs.filter(x => x.diff.length == desiredLength)[0]
}

test('returns matching letters of 2 strings', t => {
  let result = diff("abcd", "qbcf");
  t.is(result, "bc");
});

test('diff compares positionally', t => {
  let result = diff("abca", "bacd");
  t.is(result, "c");
});
...
test('part2 puzzle', t => {
  ...
  t.is(result.diff, 'qcslyvphgkrmdawljuefotxbh');
});